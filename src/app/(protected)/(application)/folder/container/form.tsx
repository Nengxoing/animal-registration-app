...
export const FolderForm: React.FC<FolderFormProps> = ({ form, onSubmit }) => {
  ...
  return (
    <Form formInstance={form} onSubmit={onSubmit} title={formTitle} subtitle={formSubtitle}>
      ...
      <Separator />
      <div className="space-y-4">
        <h3 className="text-lg font-medium">ປະເພດສັດ</h3>
        {variantFields.map((field, index) => (
          <div key={field.id} className="p-4 border rounded-lg space-y-4">
            <h4 className="font-medium">ປະເພດສັດ {index + 1}</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Form.Field name={`folderPrice.${index}.priceId`} control={form.control} label="ປະເພດສັດ" >
                <Form.Input.Combobox placeholder="ປະເພດສັດ" className="w-full" options={priceOptions} />
              </Form.Field>
            </div>
